    ...
    with torch.no_grad():
        ...
        if hasattr(Gs, 'task') and Gs.task == 'node' and v_t is not None:
            ...
            if y_ref is None:
                ...
                # Use the relabeled target node ID in the subgraph
...
def _candidate_by_edmonds(H: Data, root: Optional[int], emb: Optional[Tensor], noise_std: float = 1e-3):
    ...
    try:
        ...
        ed = nx.algorithms.tree.Branching(G)  # Edmonds/Chu–Liu
        # `ed` finds optimum by default kind='max', style='arborescence'
        Ar = ed.find_optimum(attr='weight', default=0, kind='max', style='arborescence')

        # If a specific root is requested, keep the reachable component from that root.
        if root is None:
            # pick a root with max out-degree in Ar
        ...
        if root in Ar:
            ...
            while dq:
                u = dq.popleft()
                for _, v, data in Ar.out_edges(u, data=True):
                    eid = data.get('_eid', None)
                    if eid is not None:
                        edge_mask[eid] = True
                    if v not in seen:
